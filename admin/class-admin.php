...
class Admin {
    ...
    public function init() {
        ...
        add_action('wp_ajax_preload_assist_save_facet_values', array($this, 'ajax_save_facet_values'));
        add_action('wp_ajax_preload_assist_update_parameter_position', array($this, 'ajax_update_parameter_position'));
    }
    ...
    public function enqueue_scripts($hook_suffix) {
        ...
        // Deregister any existing jQuery UI
        wp_deregister_script('jquery-ui-core');
        wp_deregister_script('jquery-ui-widget');
        wp_deregister_script('jquery-ui-tabs');
        wp_deregister_script('jquery-ui-accordion');
        
        // Re-register jQuery UI components in correct sequence
        wp_enqueue_script('jquery');
        wp_enqueue_script('jquery-ui-core', '', array('jquery'), '1.13.2', true);
        wp_enqueue_script('jquery-ui-widget', '', array('jquery-ui-core'), '1.13.2', true);
        wp_enqueue_script('jquery-ui-tabs', '', array('jquery-ui-widget'), '1.13.2', true);
        wp_enqueue_script('jquery-ui-accordion', '', array('jquery-ui-widget'), '1.13.2', true);
    ...
    public function ajax_import_facets() {
        ...
        $result = $this->facet_manager->import_facets($json_string);
    ...
    public function ajax_toggle_facet() {
        ...
        if ($enabled) {
            $success = $this->facet_manager->enable_facet($facet_name);
        } else {
            $success = $this->facet_manager->disable_facet($facet_name);
        }
    ...
    public function ajax_save_parameter() {
        ...
        $success = $this->db_manager->save_parameter($param_name, $param_values, $enabled, $position);
    ...
    /**
     * AJAX: Save facet values.
     */
    public function ajax_save_facet_values() {
        check_ajax_referer('preload_assist_nonce', 'nonce');
        ...
        $category_id = isset($_POST['category_id']) ? intval($_POST['category_id']) : 0;
        $facet_name = isset($_POST['facet_name']) ? sanitize_text_field($_POST['facet_name']) : '';
        $selected_values = isset($_POST['selected_values']) ? json_decode(stripslashes($_POST['selected_values']), true) : array();
        $all_values = isset($_POST['all_values']) ? json_decode(stripslashes($_POST['all_values']), true) : array();

        if (empty($category_id) || empty($facet_name)) {
            wp_send_json_error(array('message' => __('Category ID and facet name are required.', 'preload-assist')));
        }

        $success = $this->db_manager->save_facet_values($category_id, $facet_name, $selected_values, $all_values);

        if ($success) {
            wp_send_json_success();
        } else {
            wp_send_json_error(array('message' => __('Failed to save facet values.', 'preload-assist')));
        }
    }
    
    /**
     * AJAX: Update parameter position.
     */
    public function ajax_update_parameter_position() {
        check_ajax_referer('preload_assist_nonce', 'nonce');
        ...
        $success = $this->db_manager->update_parameter_position($param_name, $position);

        if ($success) {
            wp_send_json_success();
        } else {
            wp_send_json_error(array('message' => __('Failed to update parameter position.', 'preload-assist')));
        }
    }
}